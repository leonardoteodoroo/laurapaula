    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-dark-graphite">
      ...
      <div className="absolute inset-0 z-0">
        <img
          ...
          className={`w-full h-full object-cover transition-opacity duration-[2000ms] ease-out ${isLoaded ? 'opacity-90' : 'opacity-0'
            }`}
          fetchPriority="high"
        ...
        <div
          className={`absolute inset-0 bg-dark-graphite/40 transition-opacity duration-[5000ms] ease-out ${startEffects ? 'opacity-100' : 'opacity-0'
            }`}
        />

        {/* Dynamic Darkening Overlay - Focuses on bottom (foot) */}
        <div
          className={`absolute inset-0 bg-gradient-to-t from-dark-graphite via-dark-graphite/70 to-transparent transition-opacity duration-[5000ms] ease-out ${startEffects ? 'opacity-100' : 'opacity-0'
            }`}
        />
      </div>
      ...
      <div className="container mx-auto px-4 z-10 pt-24 pb-10 flex flex-col items-center text-center">
        ...
        <h1 className={`font-serif text-4xl md:text-5xl lg:text-7xl leading-tight text-off-white mb-6 max-w-4xl transition-all duration-1000 ${startEffects ? 'opacity-0 animate-[fadeInUp_4s_ease-out_0.5s_forwards]' : 'opacity-0'}`}>
          Quando foi a última vez que você se sentiu segura para <span className="italic text-emerald-500">usar uma sandália?</span>
        </h1>

        <div className={`max-w-2xl mb-10 transition-all duration-1000 ${startEffects ? 'opacity-0 animate-[fadeInUp_4s_ease-out_1.8s_forwards]' : 'opacity-0'}`}>
        ...
        <div className={`transition-all duration-1000 ${startEffects ? 'opacity-0 animate-[fadeInUp_4s_ease-out_4s_forwards]' : 'opacity-0'}`}>
          <a
            href={WHATSAPP_LINK}
            target="_blank"
